...
public abstract class UseAnimMixin {
    ...
    static {
        $VALUES = Stream.concat(Arrays.stream($VALUES), Stream.of(UseAnimMixin.useAnim$invokeInit(SourceOfAnnihilationMod.MODID + ":BLOCK", $VALUES[$VALUES.length - 1].ordinal() + 1)))
                .toArray(UseAnim[]::new);
    }
}